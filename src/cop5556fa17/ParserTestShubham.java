...
public class ParserTestShubham {
	...
	*/
	...
	e){System.out.println(e.getMessage());throw
	  e;}
	}
	}
	...
	*/
	...
	void statementTest1()
	throws
	SyntaxException, LexicalException {
	String k
	= "2*2+5+x!=2*2+5+x";
	String
	EXPinput
	= k+"?"+k+":"+k;
	String[] input2
	= {"@"+EXPinput,"\"STRING_LITERAL\"",
	"d6a09aa6aa42d705e4f7b01eaa57a671f05da686ded99c77294d3135563820a" };
	String[] input
	= {"img1->"+"output",
	"img1->"+"SCREEN",//
	  for imageInStatement
	"img1<-"+input2[0],
	"img1<-"+input2[1],
	// for
	imageOUTStatement
	...
	*/
	@Test
	public
	void assignmentStatementTest1()
	throws
	SyntaxException, LexicalException {
	String k
	= "2*2+5+x>2*2+5+x=,=d6a09aa6aa42d705e4f7b01eaa57a671f05da686ded99c77294d3135563820a>=2*2+5+x!=2*2+5+x<=2*2+5+x|2*2+5+x>2*2+5+x==2*2+5+x<2*2+5+x&2*2+5+x>=2*2+5+x!=2*2+5+x<=2*2+5+x";
	String
	EXPinput
	= k+"?"+k+":"+k;
	String[] input
	...
	* This example invokes the
	method for expression directly.
	* Effectively, we are
	viewing Expression as the start
	* symbol of a
	sub-language.
	*
	* Although a compiler will
	always call the parse() method,
	* invoking others is useful
	to support incremental development.
	* We will only invoke
	expression directly, but
	* following this example
	with others is recommended.
	*
	* @throws
	SyntaxException
	* @throws
	...
	* PLEASE DELETE THIS TEST
	CASE AFTER TESTING YOUR PROGRAM
	* @throws
	SyntaxException
	* @throws
	LexicalException
	...
	*/
	...
	/**
	Do NOT SHARE WITH ANYONE or I am going to kick your ass when
	I find out.
	cheers,
	Akshay Sharma
	* these test cases invoke
	...
	*/
	@Test
	public
	void andExpressionTest1()
	throws
	SyntaxException, LexicalException {
	String[] input
	= {"2*2+5+x>2*2+5+x==2*2+5+x<2*2+5+x&2*2+5+x>=2*2+5+x!=2*2+5+x<=2*2+5+x"};
	try{
	for(String x
	: input){
	show(x);
	...
	*/
	...
	// Negative test
	cases for FUnctionApplicationtest
	...
	*/
	@Test
	public
	void identOrPixelSelectorExpressionTest1()
	throws
	SyntaxException, LexicalException {
	String[] input
	= {"akshay","i[i,j]34"};
	...
	*/
	...
	// Testcase to
	invoke functionName() negatively

	@Test
	public
	void FunctionNametest2()
	throws
	SyntaxException, LexicalException {
	String[] input
	= {"x"};
	...
	*/
	...
	// this test case
	invokes lhsSelector() negatively

	@Test
	public
	void lhsSelectorTest3()
	throws
	SyntaxException, LexicalException {
	String
	input
	...
	/**
	* this positive test case
	invokes the method for SimpleParser.XySelector
	directly
	*/
	@Test
	public
	void PositiveXySelectorTest()
	throws
	SyntaxException, LexicalException {
	String
	input
	...
	/**
	* this negative test case
	invokes the method for SimpleParser.XySelector
	directly
	*/
	@Test
	public
	void NegativeXySelectorTest()
	throws
	SyntaxException, LexicalException {
	String
	input
	...
	/**
	* this positive test case
	invokes the method for SimpleParser.RaSelector
	directly
	*/
	@Test
	public
	void PositiveRaSelectorTest()
	throws
	SyntaxException, LexicalException {
	String
	input
	...
	*/
	...
	/**
	* tests
	SimpleParser.Selector only for terminal
	symbols
	*/
	@Test
	public
	...
	SyntaxException, LexicalException {
	String
	s="2*2+5+x>2*2+5+x==d6a09aa6aa42d705e4f7b01eaa57a671f05da686ded99c77294d3135563820a>=2*2+5+x!=2*2+5+x<=2*2+5+x|2*2+5+x>2*2+5+x==2*2+5+x<2*2+5+x&2*2+5+x>=2*2+5+x!=2*2+5+x<=2*2+5+x";
	String[] input1
	= {s,s+"?"+s+":"+s};
	String
	input=input1[0]+","+input1[1];
	show(input);
	Scanner
	scanner
	= new
